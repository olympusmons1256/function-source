...
import {HistoricParticipant, Organization, Participant} from "../lib/docTypes";
import {getHistoricParticipants, getOrganizationRef, getOrganizations, getUser} from "../lib/documents/firestore";
import * as fs from "fs";
import {createArrayCsvWriter} from "csv-writer";

type ParticipantWithDocIdAndRoomId = [string, string, Participant];
type HistoricParticipantWithDocIdAndRoomId = [string, string, HistoricParticipant];
...
async function getHistoricParticipantDetails(participantsWithDocIds: HistoricParticipantWithDocIdAndRoomId[]) {
  return (await Promise.all(participantsWithDocIds.flatMap(async (participantWithDocId) => {
    const [docId, roomId, participant] = participantWithDocId;
    ...
    const duration = () => {
      if (participant.deleted == undefined || participant.created == undefined) {
        return 0;
      } else {
        try {
          return Math.abs(participant.deleted.seconds - participant.created.seconds);
        } catch (e: any) {
          return 0;
        }
      }
...
async function getAllHistoricParticipants(organizationId: string, afterTimeUTC?: Date, beforeTimeUTC?: Date, spaceId?: string) {
  ...
  const historicRoomIds = (await query.get()).docs.map((doc) => doc.id);
  return (await Promise.all(historicRoomIds.map(async (roomId) => {
    ...
    if (participants == undefined) {
      ...
      return participants.flatMap(([pD, p]) => {
        if (pD == undefined || p == undefined) {
          ...
          const record = [[pD.id, pD.ref.parent.parent?.id, p] as ParticipantWithDocIdAndRoomId];
          ...
          const afterSecondsCondition = (afterTimeUTC == undefined) ? true : p.created.seconds > admin.firestore.Timestamp.fromDate(afterTimeUTC).seconds;
          const beforeSecondsCondition = (beforeTimeUTC == undefined) ? true : p.created.seconds < admin.firestore.Timestamp.fromDate(beforeTimeUTC).seconds;
...
async function calculateUsage(organization: Organization, organizationId: string, startDate: Date, endDate: Date, name?: string, spaceId?: string) {
  ...
  async function writeReportToCsv(path: string): Promise<void> {
    const entries = Object.entries(uniqueHistoricParticipants).map(([k, v]) => {
      return [
        k,
        (v.email.startsWith("guest") && v.email.endsWith("@newgameplus.live")) ? "N/A (Guest)" : v.email,
        v.name,
  ...
  async function writeDetailsToCsv(path: string): Promise<void> {
    ...
    const csvWriter = createArrayCsvWriter({
      header: ["UserId", "Email", "Name", "RoomId", "First Online", "Last Online", "Streaming Minutes"],
      path,
    });
    return await csvWriter.writeRecords(entries);
  }
...
export async function calculateUsageSpecificEvent() {
  ...
  const startOfEvent = new Date("2023-05-09T11:00:00.000-04:00");
  const endOfEvent = new Date("2023-05-10T17:00:00.000-04:00");
  organizations.map(async ([organizationRef, organization]) => {
    if (organizationRef?.id == undefined || organization == undefined) {
      console.error("ERROR: organization is undefined: ", organizationRef?.id);
      return;
    }
    if (organizationRef.id != "1xHq73UhZliwPyFzrPo0" || organization.name == undefined) {
      return;
    }
    return await calculateUsage(organization, organizationRef.id, startOfEvent, endOfEvent, "AdAge2023-05-09", "p4wo5VvPU72BqocRB9ob21");
  });
}
...
export async function calculateUsageAllTime() {
  ...
  const startOfEvent = new Date("2022-01-01T00:00:00Z");
