...
(async () => {
  ...
  const spaces = await (async () => {
    ...
    const allSpaces = await admin.firestore().collectionGroup("spaces").get();
    return allSpaces.docs.flatMap((doc) => (spaceIds != undefined && spaceIds.includes(doc.id)) ? [{id: doc.id, space: doc.data() as OrgSpace}] : [] );
  })();
  ...
  const unrealProjects =
    unrealProjectDocs
      ...
      .flatMap((o) => (spaces == undefined || unrealProjectsFromSpaces.includes(o.doc.id)) ? [o] : [])
      .flatMap((o) => (organizationId == undefined || organizationId == o.unrealProject.organizationId) ? [o] : [])
  ...
  const f = async (outOfDate: OutOfDateVersionDoc) => {
    try {
      console.debug(`Created new UPV to rebuild ${outOfDate.doc.ref.path}`);
